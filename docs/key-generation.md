...
Add on-device biometric key generation and payload signing to `react-native-simple-biometrics`. The private key lives in secure hardware (iOS Secure Enclave / Android Keystore) and cannot be used without passing biometric authentication.
...
type BiometricKey = {
  ...
  algorithm: 'RSA';    // reserved for future; constant for now
};
...
RSA-2048, payload hashed with SHA-256 internally (payload is a UTF-8 string), signed with `SHA256withRSA` (Android) / `RSASignatureMessagePKCS1v15SHA256` (iOS). Android keys also get `ENCRYPT`/`DECRYPT` purposes so future encryption support is non-breaking.
...
5. **`SimpleBiometrics.podspec`** — add `s.framework = 'Security'`.
